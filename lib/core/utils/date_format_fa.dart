...
bool _isValidGregorianDate(int year, int month, int day) {
  if (month < 1 || month > 12 || day < 1 || day > 31) return false;
  final date = DateTime(year, month, day);
  return date.year == year && date.month == month && date.day == day;
}

bool _isValidJalaliDate(int year, int month, int day) {
  if (month < 1 || month > 12 || day < 1 || day > 31) return false;
  final converted = gregorianToJalali(jalaliToGregorian(year, month, day));
  return converted.year == year && converted.month == month && converted.day == day;
}
...
DateTime? parsePersianDateInput(String input) {
  ...
  if (numeric != null) {
    ...
    if (y >= 1300 && y <= 1600 && _isValidJalaliDate(y, m, d)) {
      return jalaliToGregorian(y, m, d);
    }
    if (y >= 1900 && y <= 2200 && _isValidGregorianDate(y, m, d)) {
  ...
  for (var i = 0; i < _jalaliMonthNames.length; i++) {
    ...
    if (day != null && year != null && _isValidJalaliDate(year, i + 1, day)) {
