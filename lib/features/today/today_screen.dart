...
class TodayScreen extends ConsumerWidget {
  ...
  Future<_TodayDashboardData> _loadDashboard(AppDatabase db) async {
    ...
    return _TodayDashboardData(
      ...
      financeItems: results[2] as List<FinanceItem>,
      cases: results[3] as List<Case>,
  ...
  Widget _stableErrorCard(String title) => _stableInfoCard(
        ...
        title: title,
        message: 'این بخش هنوز آماده نمایش نیست. چند لحظه بعد دوباره تلاش کن یا به صفحه قبل برگرد.',
      );
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('امروز من'), actions: const [GlobalSearchButton()]),
      body: ListView(
        ...
        children: [
          Card(
            child: ListTile(
              ...
              subtitle: const Text('نمای تصمیم‌ساز امروز؛ کار، مهلت، مالی و پرونده‌های نیازمند توجه.'),
            ),
          ),
          ...
          FutureBuilder<_TodayDashboardData>(
            ...
            builder: (context, snapshot) {
              ...
              final todaySessions = data.timelineEvents.where((e) => e.eventType == 'جلسه' && _sameDay(e.eventDate, today)).toList();
              final casesWithoutNext = data.cases.where((c) => c.status != 'مختومه' && (c.nextAction ?? '').trim().isEmpty).toList();
              final todayFinance = data.financeItems.where((i) => _sameDay(i.date, today)).toList();
              final income = todayFinance.where((i) => i.type == 'درآمد').fold<double>(0, (s, i) => s + i.amount);
              final expense = todayFinance.where((i) => i.type == 'هزینه').fold<double>(0, (s, i) => s + i.amount);
                  ...
                  child: Column(
                    ...
                    children: [
                          ...
                          _MetricChip(
                            ...
                            onTap: () => _showDetailsSheet(
                              ...
                              todayTasks.map((task) => Card(child: ListTile(
                                ...
                                onTap: () => _openPage(context, const TasksScreen()),
                              ))).toList(),
                          ...
                          _MetricChip(
                            ...
                            onTap: () => _showDetailsSheet(
                              ...
                              [...expiredDeadlines, ...todayDeadlines].map((d) => Card(child: ListTile(
                                ...
                                title: Text(d.title),
                                subtitle: Text('${formatPersianLongDate(d.dueDate)} | ${d.deadlineType ?? 'مهلت'} | ${d.priority}'),
                                trailing: const Icon(Icons.chevron_left),
                                onTap: () => _openPage(context, const DeadlinesScreen()),
                              ))).toList(),
                      ...
                      ListTile(
                        ...
                        title: Text('مالی امروز: درآمد ${_money(income)} | هزینه ${_money(expense)}'),
                        subtitle: Text('مانده امروز: ${_money(income - expense)}'),
                        trailing: const Icon(Icons.chevron_left),
                        onTap: () => _showDetailsSheet(
                          context,
                          'مالی امروز',
                          todayFinance.map((i) => Card(child: ListTile(
                            leading: Icon(i.type == 'درآمد' ? Icons.trending_up : Icons.trending_down),
                            title: Text(i.title),
                            subtitle: Text('${i.type} | ${_money(i.amount)} | ${i.category ?? 'بدون دسته'}'),
                            trailing: const Icon(Icons.chevron_left),
                            onTap: () => _openPage(context, const FinanceScreen()),
                          ))).toList(),
                          'ثبت مالی برای امروز وجود ندارد.',
                        ),
                      ),
                    ],
                  ),
          ...
          FutureBuilder(
            ...
            builder: (context, snapshot) {
              ...
              final financeForSuggestions = snapshot.data![2] as List<FinanceItem>;
              ...
              return Column(
                ...
                children: [
                  _Section(
                    ...
                    children: todayTasks.map((task) => CheckboxListTile(
                      value: task.isDone,
                      title: Text(task.title),
                      subtitle: Text('اولویت: ${task.priority}'),
                ...
                children: [
                  _Section(
                    ...
                    children: expired.map((d) => ListTile(
                      ...
                      title: Text(d.title),
                      subtitle: Text('${formatPersianLongDate(d.dueDate)} | ${d.deadlineType ?? 'مهلت'}'),
                      trailing: const Icon(Icons.chevron_left),
                      onTap: () => _openPage(context, const DeadlinesScreen()),
                    )).toList(),
                  ),
                  _Section(
                    ...
                    children: todayDeadlines.map((d) => ListTile(
                      leading: const Icon(Icons.alarm),
                      title: Text(d.title),
                      subtitle: Text('${d.deadlineType ?? 'مهلت'} | اولویت: ${d.priority}'),
                      trailing: const Icon(Icons.chevron_left),
                      onTap: () => _openPage(context, const DeadlinesScreen()),
                    )).toList(),
                  ),
          ...
          StreamBuilder<List<FinanceItem>>(
            stream: db.select(db.financeItems).watch(),
            builder: (context, snapshot) {
              if (snapshot.hasError) {
                return _Section(
                  title: '💰 مالی امروز',
                  emptyText: 'در دریافت اطلاعات مالی خطا رخ داد؛ دوباره تلاش کنید.',
                  children: const [],
                );
              }
              final items = List<FinanceItem>.of(snapshot.data ?? const <FinanceItem>[])
                  .where((i) => _sameDay(i.date, today))
                  .toList();
              ...
              return _Section(
                title: '💰 مالی امروز',
                emptyText: 'ثبت مالی برای امروز وجود ندارد.',
                footer: items.isEmpty
                    ? null
                    : 'درآمد: ${toPersianDigits(income.toStringAsFixed(0))} تومان | هزینه: ${toPersianDigits(expense.toStringAsFixed(0))} تومان | مانده: ${toPersianDigits((income - expense).toStringAsFixed(0))} تومان',
                children: items.reversed.map((i) => ListTile(
                  leading: Icon(i.type == 'درآمد' ? Icons.trending_up : Icons.trending_down),
                  title: Text(i.title),
                  subtitle: Text('${i.type} | ${toPersianDigits(i.amount.toStringAsFixed(0))} تومان | ${i.category ?? 'بدون دسته'}'),
                  trailing: const Icon(Icons.chevron_left),
                  onTap: () => _openPage(context, const FinanceScreen()),
                )).toList(),
              );
          ...
          StreamBuilder<List<Case>>(
            ...
            builder: (context, snapshot) {
              ...
              final cases = List<Case>.of(snapshot.data ?? const <Case>[])
                  .where((c) => c.status != 'مختومه' && (c.nextAction ?? '').trim().isEmpty)
                  .toList();
              return _Section(
                title: '⚖️ پرونده‌های بدون اقدام بعدی',
...
class _TodayDetailsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text(title), actions: const [GlobalSearchButton()]),
      body: ListView(
        padding: const EdgeInsets.fromLTRB(16, 16, 16, 28),
        children: [
          Card(
...
class _Section extends StatelessWidget {
  const _Section({required this.title, required this.emptyText, required this.children, this.footer, this.muted = false});
        ...
        children: [
          ...
          if (footer != null)
            Padding(
              ...
              child: Align(alignment: Alignment.centerRight, child: Text(footer!, style: const TextStyle(fontWeight: FontWeight.bold))),
            ),
        ],
      ),
    );
