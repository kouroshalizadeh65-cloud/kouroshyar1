...
class DeadlinesScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('مهلت‌ها'), actions: const [GlobalSearchButton()]),
      body: StreamBuilder(
            ...
            child: Column(
              children: [
                ...
                ListTile(
                  ...
                  onTap: () async {
                    final input = await _askPersianDate(context, selectedDate);
                ...
                const SizedBox(height: 8),
                const Text(
                  'تاریخ را شمسی وارد کن؛ مثال: ۱۴۰۵/۰۴/۲۰ یا فردا.',
  ...
  Future<DateTime?> _askPersianDate(BuildContext context, DateTime current) async {
    final controller = TextEditingController(text: formatSimpleDate(current));
    return showDialog<DateTime>(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('تاریخ شمسی مهلت'),
        content: TextField(
    ...
    return Card(
      child: CheckboxListTile(
        ...
        onChanged: (value) async {
          await db.update(db.deadlines).replace(item.copyWith(isDone: value ?? false));
        },
      ),
    );
  }
